...
dx run /lohdata/resources/hts-knife \
    ...
    -icmd="set -euo pipefail && WGS_GROUP=$WGS_GROUP BATCH=$BATCH NGS=$NGS SEX=1 /usr/bin/time -v bash run_pipeline.sh mLOXY run_extractADs_chrX.sh 2>&1 | tee $NAME.$WGS_GROUP.batch$BATCH.dx_run.log" \
