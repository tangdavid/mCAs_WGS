...
dx run /lohdata/resources/hts-knife \
    ...
    -icmd="set -euo pipefail && WGS_GROUP=$WGS_GROUP BATCH=$BATCH /usr/bin/time -v bash run_pipeline.sh make_calls run_make_calls.sh 2>&1 | tee $WGS_GROUP.batch$BATCH.$NAME.dx_run.log" \
