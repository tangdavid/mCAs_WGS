...
OUT_PFX=$OUT_DIR/ID.variant.mask.read_phase.statistical_phase.FDR.01
...
NR>1 {
    ...
    if(!(gene in AF)) next;
    counter+=1;
    print chr,pos,gene,mask[gene],AF[gene];
} ' /mnt/project/lohdata/david/mCAs_WGS/cisGWAS/results/CN-LOH.GWAS.FDR.01.txt \
    | sort -k1,1V -k2,2n \
    | parallel --colsep='\t' --retries 4 --halt-on-error 2 -j 1 "CHR={1} GENE={3} MASK={4} AF={5} bash refine_phase.sh" \
    | tee $OUT_PFX.txt  \
    | awk -v OFS='\t' '
BEGIN {
...
END { 
    ...
    | gzip > $OUT_DIR/FDR.01.burden.allelic_shift.GWAS.txt.gz
